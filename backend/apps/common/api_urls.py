...
from backend.apps.trading.views import MT5HealthView, BrokerProfileViewSet,BrokerSettingViewSet,TradingAccountViewSet,TradingSymbolViewSet,WatchlistViewSet,SignalViewSet,OrderViewSet,OpenPositionViewSet,ClosedTradeViewSet,TradeJournalViewSet
...
urlpatterns=[path("auth/token/",TokenObtainPairView.as_view()),path("auth/token/refresh/",TokenRefreshView.as_view()),path("auth/token/verify/",TokenVerifyView.as_view()),path("mt5-health/",MT5HealthView.as_view(),name="mt5-health"),path("",include(router.urls))]
